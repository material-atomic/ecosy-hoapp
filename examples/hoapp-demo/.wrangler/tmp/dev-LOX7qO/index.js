...
var Router = class _Router {
  ...
  assign(app2, method, route, desc) {
    ...
    _Router.registries.forEach((registry) => {
      let fullPath = route;
      if (this.base) {
        fullPath = `${this.base}${route === "/" || route === "*" ? "" : route}`;
      }
      registry.register(method.toUpperCase(), fullPath, desc);
    });
  }
  getRouter() {
    const app2 = _Router.create(this.base);
    this.guard(app2);
    ["all", "get", "post", "put", "patch", "delete", "options"].forEach((method) => {
      ...
      for (const [route, handle] of handlers.entries()) {
        this.assign(app2, method, route, handle);
      }
    });
    return app2;
  }
...
// .wrangler/tmp/bundle-pLs1vP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pLs1vP/middleware-loader.entry.ts
