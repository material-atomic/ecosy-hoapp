...
// ../ecosy-hoapp/src/result.ts
var HttpResult = class {
  constructor(success, statusText, statusCode, message, data = null, errorDetail = null) {
...
var HttpError = class extends HTTPException {
  ...
  result;
  constructor(statusText, statusCode, message, errorDetail = null) {
    super(statusCode, { message });
    this.result = new HttpResult(false, statusText, statusCode, message, null, errorDetail);
  }
...
var InternalServerError = class extends HttpError {
  ...
  constructor(message, errorDetail = null) {
...
var HttpSuccess = class {
  static {
    __name(this, "HttpSuccess");
  }
  result;
  constructor(statusText, statusCode, message = "Successful", data = null) {
    this.result = new HttpResult(true, statusText, statusCode, message, data, null);
  }
  json(ctx) {
    return this.result.json(ctx);
  }
};
var OK = class extends HttpSuccess {
  static {
    __name(this, "OK");
  }
...
var UnsupportedMediaType = class extends HttpError {
  static {
...
var Router = class _Router {
  ...
  static registries = /* @__PURE__ */ new Set();
  routes = /* @__PURE__ */ new Map();
  middlewares = /* @__PURE__ */ new Map();
  childRouters = /* @__PURE__ */ new Set();
  static registry(registry) {
    _Router.registries.add(registry);
    return _Router;
  }
  route(method, ...desc) {
    ...
    if (!this.routes.has(method)) {
      this.routes.set(method, /* @__PURE__ */ new Map());
    }
    this.routes.get(method).set(routePath, reDesc);
    return this;
  }
  get(...desc) {
  ...
  use(...desc) {
    ...
    if (!this.middlewares.has("all")) {
      this.middlewares.set("all", /* @__PURE__ */ new Map());
    }
    this.middlewares.get("all").set(routePath, reDesc);
    return this;
  }
  add(...routers) {
  ...
  guard(app2) {
    ...
    if (all) {
      for (const [route, handle] of all.entries()) {
        app2.use(route, ...handle.forRoute());
      }
    }
  }
  assign(app2, method, route, desc, parentPath) {
    const handlers = [
      route,
      ...desc.forRoute()
    ];
    const methods = {
      get: /* @__PURE__ */ __name(() => app2.get(...handlers), "get"),
      post: /* @__PURE__ */ __name(() => app2.post(...handlers), "post"),
      put: /* @__PURE__ */ __name(() => app2.put(...handlers), "put"),
      patch: /* @__PURE__ */ __name(() => app2.patch(...handlers), "patch"),
      delete: /* @__PURE__ */ __name(() => app2.delete(...handlers), "delete"),
      options: /* @__PURE__ */ __name(() => app2.options(...handlers), "options"),
      all: /* @__PURE__ */ __name(() => app2.all(...handlers), "all")
    };
    if (method in methods) {
      methods[method]();
    }
    _Router.registries.forEach((registry) => {
      let fullPath = `${parentPath}${route === "/" || route === "*" ? "" : route}`;
      registry.register(method.toUpperCase(), fullPath, desc);
    });
  }
  getRouter(fullBasePath = "") {
    const app2 = _Router.create();
    this.guard(app2);
    ["all", "get", "post", "put", "patch", "delete", "options"].forEach((method) => {
      ...
      for (const [route, handle] of handlers.entries()) {
        this.assign(app2, method, route, handle, fullBasePath);
      }
    });
    for (const router of this.childRouters) {
      const childPath = router.base ?? "/";
      app2.route(childPath, router.getRouter(`${fullBasePath}${childPath === "/" ? "" : childPath}`));
    }
    return app2;
  }
  static add(app2, ...routers) {
    for (const router of routers) {
      const base = router.base ?? "/";
      app2.route(base, router.getRouter(base === "/" ? "" : base));
    }
  }
  static notFound(ctx) {
    return new NotFound("URL not found").result.json(ctx);
  }
  static error(e, ctx) {
    if (e && typeof e === "object" && "result" in e && e.result instanceof HttpResult) {
      return e.result.json(ctx);
    }
    const err = new InternalServerError(e instanceof Error ? e.message : String(e), e);
    return err.result.json(ctx);
  }
  static create(options = {}) {
    const opts = typeof options === "string" ? { base: options } : options;
    const app2 = new Hono2();
    app2.notFound(opts.notFound ?? _Router.notFound);
    app2.onError(opts.error ?? _Router.error);
    if (opts.base !== void 0) {
      return app2.basePath(opts.base);
    }
    return app2;
  }
};

// ../ecosy-hoapp/src/json.ts
var MUTATING = /* @__PURE__ */ new Set(["POST", "PUT", "PATCH", "DELETE"]);
var json = /* @__PURE__ */ __name(async (ctx, next) => {
  if (!MUTATING.has(ctx.req.method)) {
    await next();
    return;
  }
  const lengthHeader = ctx.req.header("content-length");
  const hasBody = lengthHeader !== void 0 && Number(lengthHeader) > 0;
  if (!hasBody) {
    await next();
    return;
  }
...
// ../ecosy-hoapp/src/cors.ts
var DEFAULT_ALLOWED_METHODS = ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"];
      ...
      for (const allowed of origins) {
        if (allowed.startsWith(".")) {
          try {
            ...
            if (url.hostname.endsWith(allowed)) {
              return origin;
            }
          } catch {
          }
...
cors2.DEFAULT_METHODS = DEFAULT_ALLOWED_METHODS;
cors2.DEFAULT_HEADERS = DEFAULT_ALLOWED_HEADERS;

// ../ecosy-hoapp/src/utils.ts
function toString(value) {
  return Object.prototype.toString.call(value);
}
__name(toString, "toString");
function isFunction(value) {
  return typeof value === "function" || ["[object Function]", "[object AsyncFunction]", "[object GeneratorFunction]"].includes(
    toString(value)
  );
}
...
function isObjectable(value) {
  return isObject(value) || isFunction(value);
}
__name(isObjectable, "isObjectable");
...
// ../ecosy-hoapp/src/clone.ts
var cloneStrateries = {
  [Date.toString()]: (data) => new Date(data.getTime()),
  [RegExp.toString()]: (data) => new RegExp(data.source, data.flags),
  [Map.toString()]: (data, cache) => {
  ...
  [ArrayBuffer.toString()]: (data) => {
    return data.slice(0);
  }
};
var Strateries = [Date, RegExp, Map, Set, ArrayBuffer];
var NumberTypedArray = [
  Int8Array,
  Uint8Array,
  Uint8ClampedArray,
  Int16Array,
...
function isConstructor(constructor, targets) {
  return targets.includes(constructor);
}
__name(isConstructor, "isConstructor");
function clone(data, cache = /* @__PURE__ */ new WeakMap()) {
  if (!isObjectable(data) || isFunction(data) || hasOwnProperty(data, "$$typeof") || data.constructor && isConstructor(data.constructor, DoNotClone)) {
    return data;
  }
  if (cache.has(data)) {
  ...
  if (data.constructor && isConstructor(data.constructor, NumberTypedArray)) {
    const itemData = data;
    const ItemClass = data.constructor;
    const result2 = new ItemClass(
      itemData.buffer.slice(0),
      itemData.byteOffset,
      itemData.byteLength
    );
    cache.set(data, result2);
    return result2;
  }
  const ItemConstructor = data.constructor;
  if (Strateries.includes(ItemConstructor)) {
    const handleName = ItemConstructor.toString();
    const handler = cloneStrateries[handleName];
    const result2 = handler(data, cache);
    cache.set(data, result2);
    return result2;
  }
  const result = Array.isArray(data) ? new (Object.getPrototypeOf(data)?.constructor || Array)() : Object.create(Object.getPrototypeOf(data));
  cache.set(data, result);
...
// ../ecosy-hoapp/src/freeze.ts
function deepFreezeInternal(obj) {
...
function freeze(data, cloneDeep = clone) {
  if (!isObject(data)) {
    return data;
  }
  const cloned = cloneDeep(data);
  return deepFreezeInternal(cloned);
}
__name(freeze, "freeze");

// ../ecosy-hoapp/src/descriptor.ts
function Descriptor(descriptor) {
  return class RouteDescriptor {
    static {
      __name(this, "RouteDescriptor");
    }
    static descriptor = freeze(descriptor);
    static handles = [];
    static use(...handles) {
      RouteDescriptor.handles = [...RouteDescriptor.handles, ...handles];
    ...
    static handle() {
      ...
      return async (ctx, next) => {
        const handle = RouteDescriptor.handles[RouteDescriptor.handles.length - 1];
        ;
        ...
        if (type === "html") return ctx.html(String(result), status);
        if (type === "text") return ctx.text(String(result), status);
        return new OK(result, "Successful");
      };
    }
    static forRoute() {
      if (!RouteDescriptor.handles.length) {
      ...
      return RouteDescriptor.handles.map((handle, index) => {
        ...
        return async (ctx, next) => {
          ...
          if (type === "html") return ctx.html(String(result), status);
          if (type === "text") return ctx.text(String(result), status);
          return new OK(result, "Successful").json(ctx);
        };
      });
    }
    static url(url) {
...
var Swagger = class {
  ...
  getRouter() {
    ...
    uiRouter.get(
      Descriptor({ url: "/style.css", summary: "Swagger CSS", tags: ["Swagger UI"], type: "text" }).use((c) => {
        c.header("Content-Type", "text/css");
        return CSS_CONTENT;
      })
    );
    uiRouter.get(
      Descriptor({ url: "/script.js", summary: "Swagger JS", tags: ["Swagger UI"], type: "text" }).use((c) => {
        c.header("Content-Type", "text/javascript");
        return JS_CONTENT;
      })
    );
    uiRouter.get(
      Descriptor({ url: "/json", summary: "Swagger JSON Specs", tags: ["Swagger Specs"] }).use((c) => {
        return c.json(this.registry.getOpenApiJson());
      })
    );
    uiRouter.get(
      Descriptor({ url: "/", summary: "Swagger Explorer", tags: ["Swagger UI"], type: "html" }).use((c) => {
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body>
  ...
  <script>
    // Khai b\xE1o url JSON cho script.js d\xF9ng
...
// ../ecosy-hoapp/src/swagger/index.ts
var SwaggerRegistry = class {
  static {
    __name(this, "SwaggerRegistry");
  }
  config;
  paths = {};
  constructor(config2) {
  ...
  register(method, path, desc) {
    const openApiPath = path.replace(/:([a-zA-Z0-9_]+)/g, "{$1}");
    if (!this.paths[openApiPath]) {
      this.paths[openApiPath] = {};
    }
    const lowerMethod = method.toLowerCase();
    const config2 = desc.descriptor;
    this.paths[openApiPath][lowerMethod] = {
      summary: config2.title || config2.summary || `${method} ${path}`,
      description: config2.summary,
      tags: config2.tags || ["Default"],
      responses: {
        [config2.status?.toString() || "200"]: {
...
// src/router/v1/system-admin/auth/login.ts
var login_default = Descriptor({
  url: "/login",
  summary: "Admin Login",
  tags: ["System Auth"]
}).use(() => ({ token: "sys-admin-token-123" }));

// src/router/v1/system-admin/auth/logout.ts
var logout_default = Descriptor({
  url: "/logout",
  summary: "Admin Logout",
  tags: ["System Auth"]
}).use(() => ({ success: true }));

// src/router/v1/system-admin/auth/index.ts
var authRouter = new Router("/auth");
authRouter.post("/login", login_default);
authRouter.post("/logout", logout_default);
var auth_default = authRouter;

// src/router/v1/system-admin/admins/list-admins.ts
function getListAdmins() {
  return [{ id: 1, name: "SuperAdmin" }];
}
__name(getListAdmins, "getListAdmins");
var list_admins_default = Descriptor({
  url: "/",
  summary: "List Admins",
  tags: ["System Mgmt"]
}).use(getListAdmins);

// src/router/v1/system-admin/admins/create-admin.ts
var create_admin_default = Descriptor({
  url: "/",
...
// src/router/v1/system-admin/admins/update-admin.ts
var update_admin_default = Descriptor({
  url: "/:id",
  summary: "Update Admin",
  tags: ["System Mgmt"]
}).use(() => ({ success: true }));

// src/router/v1/system-admin/admins/delete-admin.ts
var delete_admin_default = Descriptor({
  url: "/:id",
  summary: "Delete Admin",
  tags: ["System Mgmt"]
}).use(() => null);

// src/router/v1/system-admin/admins/index.ts
var adminsRouter = new Router("/admins");
adminsRouter.get("/", list_admins_default);
adminsRouter.post("/", create_admin_default);
adminsRouter.put("/:id", update_admin_default);
adminsRouter.delete("/:id", delete_admin_default);
var admins_default = adminsRouter;

// src/router/v1/system-admin/articles/list-articles.ts
var list_articles_default = Descriptor({
  url: "/",
  summary: "List Master Articles",
  tags: ["System Articles"]
}).use(() => [{ id: 101, title: "Master Article" }]);

// src/router/v1/system-admin/articles/create-article.ts
var create_article_default = Descriptor({
  url: "/",
...
// src/router/v1/system-admin/articles/update-article.ts
var update_article_default = Descriptor({
  url: "/:id",
  summary: "Update Master Article",
  tags: ["System Articles"]
}).use(() => ({ success: true }));

// src/router/v1/system-admin/articles/index.ts
var articlesRouter = new Router("/articles");
articlesRouter.get("/", list_articles_default);
articlesRouter.post("/", create_article_default);
articlesRouter.put("/:id", update_article_default);
var articles_default = articlesRouter;

// src/router/v1/system-admin/languages/list-languages.ts
var list_languages_default = Descriptor({
  url: "/:articleId",
  summary: "List Article Languages",
  tags: ["System Languages"]
}).use(() => [{ lang: "en", status: "published" }, { lang: "vi", status: "draft" }]);

// src/router/v1/system-admin/languages/add-language.ts
var add_language_default = Descriptor({
  url: "/:articleId",
...
// src/router/v1/system-admin/languages/index.ts
var languagesRouter = new Router("/languages");
languagesRouter.get("/:articleId", list_languages_default);
languagesRouter.post("/:articleId", add_language_default);
var languages_default = languagesRouter;

// src/router/v1/system-admin/revisions/get-revisions.ts
var get_revisions_default = Descriptor({
  url: "/:articleId",
  summary: "Get Revisions",
  tags: ["System Revisions"]
}).use(() => [{ revId: "r1", date: "2024" }]);

// src/router/v1/system-admin/revisions/restore-revision.ts
var restore_revision_default = Descriptor({
  url: "/:articleId/:revId/restore",
  summary: "Restore Revision",
  tags: ["System Revisions"]
}).use(() => ({ success: true }));

// src/router/v1/system-admin/revisions/index.ts
var revisionsRouter = new Router("/revisions");
revisionsRouter.get("/:articleId", get_revisions_default);
revisionsRouter.post("/:articleId/:revId/restore", restore_revision_default);
var revisions_default = revisionsRouter;

// src/router/v1/system-admin/users/list-users.ts
var list_users_default = Descriptor({
  url: "/",
  summary: "List Web Users",
  tags: ["System Web Users"]
}).use(() => [{ id: "u1", username: "john" }]);

// src/router/v1/system-admin/users/ban-user.ts
var ban_user_default = Descriptor({
  url: "/:id/ban",
  summary: "Ban Web User",
  tags: ["System Web Users"]
}).use(() => ({ banned: true }));

// src/router/v1/system-admin/users/index.ts
var usersRouter = new Router("/users");
usersRouter.get("/", list_users_default);
usersRouter.put("/:id/ban", ban_user_default);
var users_default = usersRouter;

// src/router/v1/system-admin/index.ts
var systemRouter = new Router("/system-admin");
systemRouter.add(auth_default, admins_default, articles_default, languages_default, revisions_default, users_default);
var system_admin_default = systemRouter;

// src/router/v1/web/auth/login.ts
var login_default2 = Descriptor({
  url: "/login",
  summary: "User Login",
  tags: ["Web Auth"]
}).use(() => ({ token: "user-token-abc" }));

// src/router/v1/web/auth/register.ts
var register_default = Descriptor({
  url: "/register",
...
// src/router/v1/web/auth/index.ts
var authRouter2 = new Router("/auth");
authRouter2.post("/login", login_default2);
authRouter2.post("/register", register_default);
var auth_default2 = authRouter2;

// src/router/v1/web/profile/get-profile.ts
var get_profile_default = Descriptor({
  url: "/",
  summary: "Get Profile",
  tags: ["Web Profile"]
}).use(() => ({ username: "john", email: "john@example.com" }));

// src/router/v1/web/profile/update-profile.ts
var update_profile_default = Descriptor({
  url: "/",
  summary: "Update Profile",
  tags: ["Web Profile"]
}).use(() => ({ success: true }));

// src/router/v1/web/profile/index.ts
var profileRouter = new Router("/profile");
profileRouter.get("/", get_profile_default);
profileRouter.put("/", update_profile_default);
var profile_default = profileRouter;

// src/router/v1/web/dashboard/get-stats.ts
var get_stats_default = Descriptor({
  url: "/stats",
  summary: "Get Dashboard Stats",
  tags: ["Web Dashboard"]
}).use(() => ({ views: 100, likes: 50 }));

// src/router/v1/web/dashboard/index.ts
var dashRouter = new Router("/dashboard");
dashRouter.get("/stats", get_stats_default);
var dashboard_default = dashRouter;

// src/router/v1/web/articles/read-articles.ts
var read_articles_default = Descriptor({
  url: "/",
  summary: "Read Articles",
  tags: ["Web Articles"]
}).use(() => [{ slug: "hello-world", title: "Hello World" }]);

// src/router/v1/web/articles/read-article-details.ts
var read_article_details_default = Descriptor({
  url: "/:slug",
  summary: "Read Article Details",
  tags: ["Web Articles"]
}).use(() => ({ slug: "hello-world", content: "..." }));

// src/router/v1/web/articles/index.ts
var articlesRouter2 = new Router("/articles");
articlesRouter2.get("/", read_articles_default);
articlesRouter2.get("/:slug", read_article_details_default);
var articles_default2 = articlesRouter2;

// src/router/v1/web/index.ts
var webRouter = new Router("/web");
webRouter.add(auth_default2, profile_default, dashboard_default, articles_default2);
var web_default = webRouter;

// src/router/v1/index.ts
var v1 = new Router("/v1");
v1.add(system_admin_default, web_default);
var v1_default = v1;
...
app.use("*", logger());
app.use("/v1/*", json);
Router.add(app, v1_default, swagger.getRouter());
var src_default = app;
...
// .wrangler/tmp/bundle-O9nJNJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-O9nJNJ/middleware-loader.entry.ts
